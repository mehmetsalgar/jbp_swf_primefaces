...
public class PrimeFacesPhaseListener implements PhaseListener {
    private static final long serialVersionUID = 6753685568758L;
    ...
    public void beforePhase(PhaseEvent phaseEvent) {
        ...
        boolean isPartialViewProcess = params.containsKey(Constants.PARTIAL_PROCESS_PARAM)
                && !params.get(Constants.PARTIAL_PROCESS_PARAM).equals("@all");
    ...
    private void handleAjaxRequest(FacesContext facesContext) throws IOException {
        ...
        if (isPPRRequest) {
            ...
            if (response instanceof ServletResponse) {
                ...
                characterEncoding = renderResponse.getCharacterEncoding();
            }
                
            ResponseWriter writer = facesContext.getResponseWriter();
            try {
                writer.write("<?xml version=\"1.0\" encoding=\"" + characterEncoding + "\"?>");
                ...
                boolean validationFailed = (maximumSeverity != null && maximumSeverity
            ...
            boolean found = facesContext.getViewRoot().invokeOnComponent(facesContext, ajaxSource,
...
//            }
    ...
    private void writeCallbackParams(FacesContext facesContext, RequestContext requestContext) throws IOException {
        ...
        for (String paramName : params.keySet()) {
            ...
            try {
                ...
                String json = isBean(paramValue) ? "{\"" + paramName + "\":" + new JSONObject(paramValue).toString()
                        + "}" : new JSONObject().put(paramName, paramValue).toString();
                writer.write(json);
    ...
    private void initPartialResponseWriter(FacesContext facesContext) {
        ...
        try {
            ...
            ResponseWriter responseWriter = renderKit.createResponseWriter(nullWriter, null,
                    characterEncoding);
            
            if (response instanceof ServletResponse) {
                ServletResponse servletResponse = (ServletResponse) response;
                responseWriter = responseWriter.cloneWithWriter(servletResponse.getWriter());
            } else if(response instanceof RenderResponse) {
                RenderResponse renderResponse = (RenderResponse) response;
                responseWriter = responseWriter.cloneWithWriter(renderResponse.getWriter());
            }
